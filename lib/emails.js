...
function specCard(rows) {
  const cells = rows
    ...
    .map(([label, value, opts]) => {
      ...
      return `<tr>
        <td style="padding:10px 16px;border-top:1px solid ${CARD_BORDER};font-family:${FONT_MONO};font-size:11px;letter-spacing:0.08em;text-transform:uppercase;color:#7d8c82;white-space:nowrap;vertical-align:top;width:42%;">${escapeHtml(label)}</td>
        <td style="padding:10px 16px;border-top:1px solid ${CARD_BORDER};font-size:13px;color:#eef2ee;line-height:1.5;">${val}</td>
      </tr>`
    })
    .join('')
  if (!cells) return ''
